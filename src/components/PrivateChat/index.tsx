...
const MessageBubble = ({ message, isCurrentUser, isAdvisor, formattedTime }: MessageBubbleProps) => {
    // Determinar classes de estilo com base no tipo de remetente
    ...
    return (
        <motion.div
            ...
            className={`rounded-2xl px-4 py-3 max-w-xs md:max-w-md lg:max-w-lg shadow-sm ${bubbleClasses}`}
        >
            ...
            <p className={`${isCurrentUser || isAdvisor ? 'text-white' : 'text-gray-800'} break-words`}>
                {message.content}
            </p>
        </motion.div>
    );
...
// Props para DateSeparator
interface DateSeparatorProps {
    date: string;
}

// Separador de data para mensagens
...
export default function PrivateChat({
    ...
    // Função auxiliar para ordenar mensagens por timestamp (do mais antigo para o mais novo)
    ...
    // Simulação de informações do chat (pode ser substituído por uma API real)
    ...
    // Inicializar token e extrair informações do usuário
    useEffect(() => {
    ...
    // Buscar mensagens com suporte à paginação
    const fetchMessages = useCallback(async (url?: string) => {
        ...
        try {
            ...
            if (isInitialLoad) {
                // Na carga inicial, exibindo as mensagens de forma crescente
                setMessages(sortMessages(data.results));
            } else {
                // Ao paginar, adiciona as mensagens antigas antes das existentes
    ...
    // Atualizar informações do chat quando as mensagens mudarem
    useEffect(() => {
    ...
    // Ajustar o scroll mantendo a posição relativa após carregar mensagens antigas
    useEffect(() => {
    ...
    // Configuração e reconexão do WebSocket
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="px-4 py-3 bg-white border-b border-gray-200 flex items-center"
            >
            ...
            >
                <div className="relative flex items-center bg-white rounded-full shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <input
                        ...
                        className="flex-1 py-3 px-2 text-gray-700 focus:outline-none bg-transparent disabled:text-gray-400"
                    />
                    <button className="p-3 text-gray-500 hover:text-blue-600 transition-colors focus:outline-none">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <motion.button
                        ...
                        className={`px-6 py-3 rounded-r-full flex items-center justify-center transition-colors ${
                            isSocketConnected && newMessage.trim() && !isSending
