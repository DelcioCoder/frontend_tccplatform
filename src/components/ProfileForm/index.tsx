...
export default function ProfileForm() {
  ...
  // Buscarando o nome de usuário via API
  useEffect(() => {
    async function fetchUsername() {
      try {
        ...
        if (response.ok) {
          ...
          console.error('Erro ao buscar nome de usuário:', await response.json());
        }
      } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-700 to-emerald-600 flex items-center justify-center p-4">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <AnimatedFormFields userType={userTypeFromHook} profile={profile} handleChange={handleChange} username={username} />
          <motion.button
